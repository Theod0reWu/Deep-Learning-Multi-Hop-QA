...
class BaseRetrieverTester:
    ...
    def __init__(self, batch_size, dataset=None, log_level=logging.INFO):
        """
        ...
        # Load dataset
        self.dataset = (
            get_whole_batch_dataset(batch_size)
    ...
    def generate_report(self, results_df, batch_size):
        """
        Generate a detailed report from test results.

        Args:
            results_df (pd.DataFrame): Results dataframe from test_retriever
            batch_size (int): Batch size used during dataset retrieval.

        Returns:
            dict: Summary statistics grouped by model and hop count categories.
        """
        report = {}

        # Add a new column to categorize by "predicted hops + n"
        results_df["hop_group"] = results_df["retrieval_iterations"] - batch_size

        # Group by model and hop group
        for model in results_df["model"].unique():
            model_results = results_df[results_df["model"] == model]

            for hop_group in sorted(model_results["hop_group"].unique()):
                hop_results = model_results[model_results["hop_group"] == hop_group]

                report[f"{model}: predicted hops + {int(hop_group)}"] = {
                    "total_queries": len(hop_results),
                    "avg_retrieved_docs": hop_results["retrieved_docs"].apply(len).mean(),
                    "avg_retrieval_iterations": hop_results["retrieval_iterations"].mean(),
                    "avg_answer_similarity": hop_results["answer_similarity"].mean(),
                    "accuracy_rate": hop_results["answer_accuracy"].mean(),
                }
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=6, help="Batch size for dataset retrieval"
    )  # <-- Added argument for batch size

    args = parser.parse_args()

    # Initialize and run tester
    tester = BaseRetrieverTester(batch_size=args.batch_size, dataset=get_whole_batch_dataset(args.batch_size))  # <-- Dynamic batch size
    ...
    # Generate and print report
    report = tester.generate_report(results, args.batch_size)
