def parse_text_metrics(content):
    ...
    for line in content.split('\n'):
        ...
        if ':' in line:
            ...
            elif not any(metric in line.lower() for metric in ['accuracy:', 'mean_similarity:']):
...
def format_json_output(overall_metrics, metrics):
    ...
    output.append("=================")
    output.append("")
    output.append("Overall GPT Metrics (using sentence-transformer for embedding similarity):")
    output.append("--------------")
    output.append("{")
    ...
    for reasoning_type, values in metrics.items():
        ...
        metric_lines.append(f'    "mean_similarity": {values.get("mean_similarity", 0)}')
        metric_lines.append("  }" + ("," if reasoning_type != list(metrics.keys())[-1] else ""))
